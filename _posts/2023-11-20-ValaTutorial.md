...
定义了一种结构类型，即复合值类型。Vala 结构体可以有私有的方法，也可以有私有成员，这意味着需要显式公共访问修饰符。
...
在本例中，我们定义了一个 `Callable` 接口，它有一个名为`abstract bool hang ()` 的 `default_hang`，它可以是静态方法或虚方法。然后，Caller 是为 TechPhone 和 Phone 类实现 Callable 的基类，而 Caller 的 hang () 方法只是简单地调用 Callable 的默认实现。TechPhone 什么也不做，只是将 Caller 作为基类，使用默认方法实现；但 Phone 重载了 Caller.hang()，这使得它使用自己的实现，即使它被转换为 Callable 对象，也可以始终调用它。
...
当执行`yield`语句时，CPU 的控制权首先传递给被调用的方法，该方法运行到第一次`yield`为止，然后返回到调用方法，该方法完成`yield`语句，然后将控制权交还给自己的调用者。
...
反之，非拥有引用不会记录在其引用的对象中。这样就可以在逻辑上应该删除对象时将其删除，而无需考虑可能仍存在对它的引用这一事实。实现这一点的通常方法是定义一个方法来返回一个非拥有引用，例如：
...
Vala 会尝试获取非拥有引用指向的实例的引用或副本。

#### 属性所有权

与普通方法不同的是，属性的返回值始终是非拥有的。这意味着你不能返回在 get 方法中创建的新对象。这也意味着，你不能使用方法调用中的自有返回值。一个令人恼火的事实是，属性值为拥有该属性的对象所有。获取该属性值的调用不应窃取或复制（通过复制或增加引用计数）对象方的值。
...
`unowned`修改器可用于使自动属性的存储空间成为非拥有存储空间。这意味着：
...
关键字 `owned` 可用于明确要求属性返回一个属性值的拥有引用，从而使属性值在对象中重现。在添加 `owned` 关键字前请三思。它是一个属性还是一个简单的 `get_xxx` 方法？你的设计也可能存在问题。总之，以下代码是正确的代码段：

```vala
public owned Object property { owned get { return new Object(); } }
```

非拥有引用的作用类似于稍后介绍的指针。不过，它们的使用要比指针简单得多，因为它们可以很容易地转换为普通引用。不过，一般来说，除非你知道自己在做什么，否则不应在程序中广泛使用它们。
...
-   作为一个类型转换操作符，它可以用来避免类的重复引用计数，这在 Vala 中通常是不可能的。例如
...
在本例中，`x` 是满足要求的固定参数。通过 `va_list()`，可以获得 varargs 列表。然后，可以在这个列表上依次调用泛型方法`arg<T>()`来获取一个又一个参数，其中 T 是参数应被解释为的类型。如果从上下文中可以明显看出类型（就像我们的示例一样），则会自动推断出类型，此时只需调用 `arg()`，而无需泛型参数。
...
Varargs 有一个值得注意的严重缺点：它们不是类型安全的。编译器无法判断你是否向方法传递了正确类型的参数。这就是为什么只有在有充分理由的情况下才考虑使用 varargs，例如：为使用 Vala 库的 C 程序员提供方便的函数或绑定 C 函数。通常，数组参数是更好的选择。
...
-   当你特别想优化程序的一部分，而[非自有拥有引用](https://wiki.gnome.org/Projects/Vala/Tutorial#Unowned_References)又不足时。
...
使用这些非`Object`类的一个明显例子就是 GLib 绑定。因为 GLib 的级别比 GObject 低，所以绑定中定义的大多数类都是此类。另外，如前所述，非Object类的轻量级使它们在许多实际情况下都非常有用（例如 Vala 编译器本身）。不过，非Object类的详细用法不在本教程的讨论范围之内。请注意，这些类与结构体有着本质区别。
