...
刷新启动事件是通过`gst_event_new_flush_start ()` 创建的。与 EOS 事件一样，它没有任何属性。通常只有驱动流水线的元素才会创建该事件，如以推送模式运行的源元素或基于拉模式的解复用器/解码器。
...
刷新停止事件是通过`gst_event_new_flush_stop ()` 创建的。它有一个参数，用于控制流水线的运行时间是否应重置为 0。通常情况下，在刷新停止后，`running_time` 会被重置为 0。
